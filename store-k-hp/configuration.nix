...
}:
{
  ...
  networking.hostName = "store-k-hp";
...
#   };
  ...
  environment.systemPackages = with pkgs; [
    ...
    git
    ntfs3g
    kdePackages.bluez-qt # bluetooth
    dmidecode
    pciutils
    btrfs-progs
    cryptsetup
  ...
  nix.gc = {
    automatic = true;
    dates = "weekly";
    options = "--delete-older-than 9m";
  };
  
  
  # backup current configuration
  ## path: /nix/store/<hash>-system-config
  ## copy: cp -r /nix/store/*-system-config /tmp/nixconfig
